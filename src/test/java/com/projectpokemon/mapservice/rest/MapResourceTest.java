...
class MapResourceTest {
    ...
    @Test
    void get_all_maps() {
        when(mockMapService.getAllMaps()).thenReturn(Arrays.asList(routeMap1, routeMap2));

        List<RouteMap> actual = mapResource.getAllMaps();
    ...
    @Test
    void get_map_by_id() {
        when(mockMapService.getMapById(anyInt())).thenReturn(routeMap1);

        RouteMap actual = mapResource.getMapById(ID_1);
